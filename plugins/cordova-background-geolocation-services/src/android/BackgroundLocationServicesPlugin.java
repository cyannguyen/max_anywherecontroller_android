...
public class BackgroundLocationServicesPlugin extends CordovaPlugin {
    ...
    // variables used to avoid application to be killed by the OS
    private PowerManager.WakeLock wakeLock;
    private Handler handler;
	private PendingIntent wakeupIntent;
	private CordovaWebView webViewPlugin;
	private PowerManager powerManager = null;
	
	private final Runnable heartbeat = new Runnable() {
	    public void run() {
	        try {
	        	//make app visible to avoid kill
	        	webViewPlugin.getView().dispatchWindowVisibilityChanged(View.VISIBLE);
    ...
    public void initialize(CordovaInterface cordova, CordovaWebView webView) {
    	...
    	//grabing the reference of cordova webview to send the alive signal to avoid it been killed by OS
		this.webViewPlugin = webView;

		super.initialize(cordova, webView);
		
		//grab power management related with cordova app
		this.powerManager = (PowerManager) cordova.getActivity().getSystemService(Context.POWER_SERVICE);
		
		//Handler to run the thread that will keep app alive
		handler = new Handler();
		//intent to send to the view 
    ...
    public boolean execute(String action, JSONArray data, CallbackContext callbackContext) {
        ...
        System.out.println("BackgroundLocationUpdatePlugin :: " + action + " :: isEnabled Flag :: " + isEnabled);

        if (ACTION_START.equalsIgnoreCase(action) && !isEnabled) {
        	
        	//this code start the handler to keep app alive
        	//TO:DO investigate the start of the handler scheduler at on Pause method
        	this.wakeLock = this.powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK , "PowerManagementPlugin");
        	this.wakeLock.acquire();
        	this.handler.postDelayed(heartbeat, 10000);
    ...
    private JSONObject locationToJSON(Bundle b) {
        ...
        try {
            ...
            try {
            	System.out.println("BackgroundLocationUpdatePlugin :: Location GPS From Plugin " + b.toString());
            } catch (Exception ex) {
            	System.out.println("Print location failed :: ERROR :: ");
            	ex.printStackTrace();
            }
