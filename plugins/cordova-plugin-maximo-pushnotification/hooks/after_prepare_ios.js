...
module.exports = context => {
  const projectRoot = context.opts.projectRoot;
    ...
    getPlatformVersionsFromFileSystem(context, projectRoot).then(platformVersions => {
      ...
      let frameworks = ["BMSAnalyticsAPI.framework", "BMSCore.framework", "BMSPush.framework", "FirebaseAuth.framework",
    "FirebaseCore.framework", "FirebaseDatabase.framework", "FirebaseFirestore.framework", "FirebaseInAppMessaging.framework",
    "FirebaseInAppMessagingDisplay.framework", "FirebaseInstanceID.framework", "FirebaseMessaging.framework", "FirebaseStorage.framework",
    "GoogleUtilities.framework", "grpc.framework", "grpcpp.framework", "GTMSessionFetcher.framework",
    "leveldb.framework", "nanopb.framework", "openssl_grpc.framework", "protobuf.framework",
    "SwiftyJSON.framework",  "abseil.framework"];
    
    //add the Embed Frameworks Build Phase to the project if it doesn't already exist
    ////if (xcodeProject.pbxEmbedFrameworksBuildPhaseObj(xcodeProject.getFirstTarget().uuid) == null) {
    ...
    frameworks.map(framework =>{
      ...
      // xcodeProject.removeFramework(path.join(projectName, 'Plugins', 'cordova-plugin-maximo-pushnotification', framework), {customFramework: true});

      // xcodeProject.addFramework(path.join(projectName, 'Plugins', 'cordova-plugin-maximo-pushnotification', framework), {customFramework: true, embed: true, sign: true});
      ...
      const pushSwiftHeaderProperty = "PushNotification-Swift.h";
      ...
      for (configName in buildConfigs) {
        if (!COMMENT_KEY.test(configName)) {
          ...
          if (xcodeProject.getBuildProperty('SWIFT_OBJC_INTERFACE_HEADER_NAME', buildConfig.name) !== pushSwiftHeaderProperty) {
            xcodeProject.updateBuildProperty('SWIFT_OBJC_INTERFACE_HEADER_NAME', pushSwiftHeaderProperty, buildConfig.name);
            console.log('Update IOS build setting SWIFT_OBJC_INTERFACE_HEADER_NAME to:', pushSwiftHeaderProperty, 'for build configuration', buildConfig.name);
          }
...
const getPushSwiftHeaderPath = (projectPath, iosPlatformVersion) => {
  let pushSwiftHeaderPath;
  
  pushSwiftHeaderPath = path.posix.join(projectPath, 'Plugins', 'cordova-plugin-maximo-pushnotification', 'MaximoPushNotificationApp-Swift.h');
  

  return pushSwiftHeaderPath;
};
