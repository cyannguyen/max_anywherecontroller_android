...
function __loadStaticAppProps(lines){
    ...
    // Penetration Test resolution - var rootUrl = props['wlServerProtocol'] + '://' + props['wlServerHost'] + ':' + props['wlServerPort'] + context;
    var rootUrl = context;
...
/*globals WL, WL_, __WL, WLJSX, WLJQ, cordova, WL_LOADER_CHECKSUM, WL_I18N_MESSAGES, WL_CLASS_NAME_TRANSLATE*/
...
var __WLUtils = function () {
  ...
  function findCordovaPath() {
    ...
    if (WL.Client.getEnvironment() === WL.Env.IPHONE || WL.Client.getEnvironment() === WL.Env.IPAD || WL.Client.getEnvironment() === WL.Env.ANDROID) {
      cordova.exec(function (path) {
        dfd.resolve(path);
      }, null, 'WLApp', 'getMainHtmlFilePath', []);
    } else {
      var path = null;
  ...
  function loadWLClientMessages(lang) {
    ...
    // Change the language code for windows environmnet
    ...
    findCordovaPath().then(function(basePath) {
      if (WL.StaticAppProps.MESSAGES_DIR) {
        url = basePath + WL.StaticAppProps.MESSAGES_DIR + '/' + lang + '/messages.json';
        if (lang === null || typeof lang === 'undefined' || lang.indexOf('en') === 0) {
          url = basePath + WL.StaticAppProps.MESSAGES_DIR + '/messages.json';
        }
      } else {
      ...
      function successCallback(file) {
        file.file(function(file) {
            ...
            reader.onloadend = function() {
                WL.ClientMessages = JSON.parse(this.result);
            };
      ...
      function failureCallback(e) {
        var msg = 'WL.Utils.loadWLClientMessages(..) failed to read JSON file';
        var ex = new Error(e);
      ...
      if (WL.Client.getEnvironment() === WL.Environment.WINDOWS8 || WL.Client.getEnvironment() === WL.Environment.WINDOWS || WL.Client.getEnvironment() === WL.Environment.WINDOWSPHONE8) {
        ...
        window.resolveLocalFileSystemURL(url, successCallback, failureCallback);
      }
    });
  }
...
__WLSimpleDialog = function() {
  ...
  this.__callback = function(result) {
    ...
    if (WL.EnvProfile.isEnabled(WL.EPField.USES_CORDOVA) == true) {
  ...
  this.show = function(title, text, buttons, options, blockPopUp) {
    ...
    } else if (WL.StaticAppProps.ENVIRONMENT && (WL.StaticAppProps.ENVIRONMENT === WL.Environment.WINDOWS8 || WL.StaticAppProps.ENVIRONMENT === WL.Environment.WINDOWS || WL.StaticAppProps.ENVIRONMENT === WL.Environment.WINDOWSPHONE8)) {
    ...
    } else if (WL.EnvProfile.isEnabled(WL.EPField.USES_CORDOVA) != "NO") {
  ...
  __logAppSession = function () {
          ...
          try {
              ...
              var metadata = {
                  ...
                  '$closedBy': 'USER',
                  //'$appSessionID': appSessionID : 65817 - Sensitive Information Disclosed in Logs
      ...
      cordova.exec(
        function (deviceId) {
          if(deviceId && deviceId.length > 0){
            requestOptions.parameters[PARAM_DEVICE_ID_KEY] = deviceId;
          }
          WLAuthorizationManager.__getWlServerUrl().then(
            function (serverUrl) {
              ...
              request = new WLJSX.Ajax.WLRequest(
                serverUrl + 'preauth/v1/preauthorize',
                requestOptions
              );
            },
            function (error) {
              WL.Logger.debug(
                'getWlServerUrl failed with error: ' + JSON.stringify(error)
              );
              dfd.reject(error);
            }
          );
        ...
        function (error) {
          WL.Logger.debug('Failed to retrieve encrypted device ID');
          dfd.reject(error);
        },
        AUTHORIZATION_MANAGER_PLUGIN_NAME,
